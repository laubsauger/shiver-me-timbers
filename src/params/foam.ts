...
export interface FoamParams {
  ...
  /**
   * distance at which a cascade's full-resolution foam retires, in ITS OWN
   * texel widths (domain/resolution). Storage textures have no mips, so a
   * sub-pixel texel read is pure aliasing — past this the coarsest band hands
   * over to its pre-filtered tier and the finer bands fade out.
   */
  cascadeFadeTexels: number;
  /** multiplier from cascade fade start to fully retired (≥ 1.05) */
  cascadeFadeSpan: number;
  /**
...
function foamParamsMeta(): Partial<Record<keyof FoamParams, ParamMeta>> {
  return {
    ...
    cascadeFadeTexels: { min: 100, max: 20000, step: 100 },
    cascadeFadeSpan: { min: 1.05, max: 10, step: 0.05 },
